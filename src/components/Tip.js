...
export function TipWarning({ children }) {
  return (
    <div className="not-prose mt-6 -mb-1 flex space-x-2">
      <svg
        className="flex-none w-5 h-5 text-rose-400 dark:text-rose-600"
        viewBox="0 0 20 20"
        fill="currentColor"
        aria-hidden="true"
      >
      ...
      <p className="text-rose-700 text-sm font-medium dark:text-rose-200">{children}</p>
    </div>
  )
}
